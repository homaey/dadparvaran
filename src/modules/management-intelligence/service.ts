...
import { NotificationType,TaskStatus } from "@/lib/content-enums";import { db } from "@/lib/db";import { getBaleProvider } from "@/modules/notifications/bale-provider";
...
async function notifyAdmins(type:NotificationType,title:string,message:string,dedupeSuffix:string,taskId?:number){const admins=await db.user.findMany({where:{role:Roles.ADMIN},select:{id:true,baleId:true}});const bale=getBaleProvider();for(const admin of admins){const dedupeKey=`admin-alert:${type}:${dedupeSuffix}:${admin.id}`;const notification=await db.notification.upsert({where:{dedupeKey},update:{},create:{userId:admin.id,taskId,type,title,message,scheduledFor:new Date(),dedupeKey}});if(notification.status!=="PENDING"||!admin.baleId||!bale)continue;try{await bale.send({chatId:admin.baleId,title,message});await db.notification.update({where:{id:notification.id},data:{channel:"BALE",status:"SENT",sentAt:new Date()}})}catch(e){console.error("Bale delivery failed",e)}}}
